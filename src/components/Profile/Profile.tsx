...
const Profile = ({ speaker, index }: ProfileProps) => {
  ...
  return (
    <div
      key={index}
      className="flex items-center gap-3 max-[1024px]:flex-1 basis-[32%] max-[1024px]:basis-[calc(50%-2rem)] max-[400px]:basis-full flex-shrink-0"
    >
      <Image
        src={imageUrl}
        alt={name}
      ...
      />
      <h5 className="max-[768px]:text-xs text-base text-[#F2BAA7] font-light flex-1">
        {name}
      </h5>
    </div>
  );
