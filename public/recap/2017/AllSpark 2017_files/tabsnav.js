...
(function (window) {
  ...
  TabsNav.prototype._init = function () {
    this.DOM.tabs = [].slice.call(
      this.DOM.el.querySelectorAll('.tabsnav__item'),
    );
    this.DOM.bars = [].slice.call(
      this.DOM.el.querySelectorAll('.tabsnav__bar'),
    );
  ...
  TabsNav.prototype._openTab = function (tab) {
    ...
    this.DOM.bars.forEach(function (bar) {
      ...
      bar.style.transformOrigin =
        self.options.layout === 'vertical' ? '0% 50%' : '50% 0%';
    });
    ...
    var animeTabs = {
        targets:
          this.options.movable === 'all'
            ? this.DOM.tabs
            : this.DOM.tabs[this.current],
      },
      animeBars = {
        targets:
          this.options.movable === 'all'
            ? this.DOM.bars
            : this.DOM.bars[this.current],
      },
        ...
        if (cnt === 1 || self.options.animedelay === 0) {
          ...
          if (self.current >= middle) {
            return index <= self.current
              ? index * self.options.animedelay
              : (total - index - 1) * self.options.animedelay;
          } else {
            return index < self.current
              ? index * self.options.animedelay
              : (total - index - 1) * self.options.animedelay;
      ...
      animeTabsTranslation = function (target, index, cnt) {
        if (index === self.current || cnt === 1) {
          ...
          return index > self.current
            ? self.dim.win - (self.dim.position + self.dim.measure) - pixels
            : -1 * self.dim.position + pixels;
        }
      },
      animeBarsScale = function (target, index, cnt) {
        return index === self.current || cnt === 1
          ? self.dim.win / self.dim.measure
          : 1;
      };
    ...
    animeTabs[
      this.options.layout === 'vertical' ? 'translateX' : 'translateY'
    ] = animeTabsTranslation;
    animeBars[this.options.layout === 'vertical' ? 'scaleX' : 'scaleY'] =
      animeBarsScale;
    ...
    if (this.extraEl) {
      var animeExtra = {
        ...
        animeExtraTranslation =
          this.options.layout === 'vertical'
            ? this.dim.win -
              (this.dim.position + this.dim.measure) +
              Math.abs(extraBounds.left - this.dim.position) +
              this.dim.measure
            : -1 * this.dim.position;
  ...
  TabsNav.prototype._closeTab = function (tab) {
    ...
    var self = this,
      ...
      animeTabsDelay = function (target, index, cnt) {
        return cnt === 1 || self.options.animedelay === 0
          ? 0
          : Math.abs(self.current - index) * self.options.animedelay;
      };
    ...
    if (this.extraEl) {
      var animeExtra = {
        ...
        easing: this.options.animeeasing,
        delay:
          Math.abs(this.current - this.totalTabs) * this.options.animedelay,
      };
  ...
  TabsNav.prototype.show = function (callback) {
    ...
    this.DOM.tabs.forEach(function (tab) {
      ...
      bar.style.transform =
        self.options.layout === 'vertical' ? 'scaleX(0)' : 'scaleY(0)';

      title.style.opacity = 0;
      title.style.transform =
        self.options.layout === 'vertical'
          ? 'translateX(10) rotate(-90)'
          : 'translateY(10)';
    });
    ...
    var animeBars = {
      ...
      complete: function () {
        ...
        self.DOM.tabs.forEach(function (tab) {
          ...
          bar.style.transform =
            self.options.layout === 'vertical' ? 'scaleX(1)' : 'scaleY(1)';
  ...
  TabsNav.prototype._resize = function () {
    ...
    if (this.isOpen) {
      ...
      this.dim.win =
        this.options.layout === 'vertical' ? win.width : win.height;
      ...
      this.DOM.tabs.forEach(function (tab, idx) {
        if (
          self.options.movable === 'all' ||
          (self.options.movable === 'single' && idx === self.current)
        ) {
          ...
          if (idx === self.current) {
            ...
            tVal =
              idx > self.current
                ? self.dim.win - (self.dim.position + self.dim.measure)
                : -1 * self.dim.position;
            sVal = 1;
          }
      ...
      if (this.extraEl) {
        this.extraEl.style.transform =
          translateStr +
          '(' +
          (this.options.layout === 'vertical'
            ? this.dim.win - (this.dim.position + this.dim.measure)
            : -1 * this.dim.position) +
          'px)';
      }
    }
